...
for i=1:iter
    n = 2*i - 1;
    anSol = anSol + 1./n.*cosh(n.*pi.*x1./W) ...
        ./cosh(n.*pi.*(L./2)./W).*sin(n.*pi.*y1./W);

    avgError(i) = mean(mean(abs(anSol.*4.*V0./pi - matsol)));
end

anSol = anSol.*4.*V0./pi;

figure(2);
surf(linspace(0,L,nx),linspace(0,W,ny),anSol);
xlabel('x');
ylabel('y');
...
%%
% 
% Analytical solution is easier to implement than numericcal although
% it is hard to find.
% 
% For complex problems, FD will be preferred as it is easier to find than the analytical solution. 
%
